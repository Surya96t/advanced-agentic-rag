...
class HealthCheckResponse(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=utc_now, description="Health check timestamp (UTC)")
...
class ErrorResponse(BaseModel):
    ...
    details: dict | None = Field(
        default=None, description="Additional error details (validation errors, etc.)"
    )
    request_id: str | None = Field(
        default=None, description="Request ID for debugging (if available)"
    )
...
class IngestResponse(BaseModel):
    ...
    chunks_created: int = Field(ge=0, description="Number of chunks created during ingestion")
    message: str = Field(default="Document ingested successfully", description="Success message")
...
class SearchResponse(BaseModel):
    ...
    results: list[SearchResult] = Field(default_factory=list, description="Array of search results")
    query: str = Field(..., description="Original search query")
    total_results: int = Field(ge=0, description="Total number of results found")
    search_type: str = Field(..., description="Type of search performed (vector/text/hybrid)")
    execution_time_ms: int = Field(ge=0, description="Search execution time in milliseconds")
